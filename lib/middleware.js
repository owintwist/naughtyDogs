...
const fs = require('fs')
const sdns = require('dns-sync')
const dns = require('dns')
const ipUtils = require('ip')
const debug = require('debug')('owintwist:naughtyDogs')
...
let badURIs = [
  ...
  /\/uploadify/i,
  /\/webconfig\.txt/i,
  /\/wp-(admin|cache|content|cros|footers|file|log(in|o)|includes?|post)/i,
  /\/xfchat/i,
  ...
  /\/(0day|1ndex|abbrevsprl|add|authenticating|configuration|css|defau1t|dumper|elements|goog1es|google-assist|head|helper|infos|injctory|install|j(user|mail)s?|jss|laj|maill|mmytc|morepaddy|m-f4r3s|plugin(_googlemap2_proxy)?|ricsky|resd|restore|robot|RoseLeif|SessionController|setup(-config)?|sfn|Socket[A-z0-9]*ontrol|how|shootme|Signedint|sqldebug|support|thumb|tmp|utility\/convert\/index|wpfootes|wsdl|xmlrp(c|z-sql)|yjh|zeubda)\.php/i,


// ?q=login.destroy.session  xmlrpz-sql.php"
// ^/tpl/user
//
// images/login/
...
module.exports = function (req, res, next) {
  ...
  let visip = req.headers['x-forwarded-for'] || req.connection.remoteAddress
  ...
  if (ipUtils.isPrivate(visip)) {
    ...
    getOut(visip)
  } else if (useragent && dontFollowRobotstxtRules.some(function (rx) { return rx.test(useragent) })) {
    res.type('text')
    res.status(403).end("You must follow robots.txt's rules. Please update your robot.")
  } else {
